		...
		if (this->warning_status_sensor_ != nullptr ||
			...
			this->parent_->register_status_information_callback_v25([this](PaceBmsProtocolV25::StatusInformation& status_information) {
				...
				if (this->raw_status_sensor_ != nullptr) {
					this->parent_->queue_sensor_update([this, value = status_information.rawStatusHex]() { this->raw_status_sensor_->publish_state(value); });
				}
			});
		}
...
void PaceBmsTextSensor::dump_config() {
	...
	LOG_TEXT_SENSOR("  ", "Raw Status", this->raw_status_sensor_);
